...
class usermodel {
    ...
    function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
        ...
        // BEGIN
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member SET uid='$uid', username='$username', password='$password', email='$email', adminid='0', groupid='10', regdate='" . $this->base->time . "', credits='0', timeoffset='9999'");
//        echo "INSERT INTO `ultraxs`.pre_common_member SET uid='$uid', username='$username', password='$password', email='$email', adminid='0', groupid='10', regdate='" . $this->base->time . "', credits='0', timeoffset='9999'";exit;
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member_status SET uid='$uid', regip='$regip', lastip='$regip', lastvisit='" . $this->base->time . "', lastactivity='" . $this->base->time . "', lastpost='0', lastsendmail='0'");
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member_profile SET uid='$uid'");
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member_field_forum SET uid='$uid'");
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member_field_home SET uid='$uid'");
        $this->db->query("INSERT INTO `ultraxs`.pre_common_member_count SET uid='$uid', extcredits1='0', extcredits2='0', extcredits3='0', extcredits4='0', extcredits5='0', extcredits6='0', extcredits7='0', extcredits8='0'");
        // END
        return $uid;
    }
    ...
    function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
        ...
        if ($ignoreoldpw) {
            if ($ignoreoldpw > 1) {//用户uid
                $user = json_decode(cookie('user_auth'), true); //获取cookie里面的用户信息
                $mobile = floatval($user["username"]); //用户名手机号
                $this->db->query("UPDATE " . UC_DBTABLEPRE . "members SET username ='$username' WHERE uid='$ignoreoldpw' and username ='$mobile'");
//                echo "UPDATE " . UC_DBTABLEPRE . "members SET username ='$username' WHERE uid='$ignoreoldpw' and username ='$mobile'";
