...
class Wechat {
    ...
    private function getAccessToken() {
        ...
        $access_token = S('access_token');// 获取缓存数据
        if( $access_token==''){
            $jsonStr = $this->http(self::AUTH_URL, $params);
            if ($jsonStr) {
                $jsonArr = $this->parseJson($jsonStr);
                if ($jsonArr) {
                    S('access_token', $jsonArr['access_token'],7200);//加入缓存
