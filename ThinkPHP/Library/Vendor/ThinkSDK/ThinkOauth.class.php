...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		$this->Type = strtoupper(substr($class, strripos($class, '\\')+1,-3));
		//获取应用配置
		$config = C("THINK_SDK_{$this->Type}");
		if(empty($config['APP_KEY']) || empty($config['APP_SECRET'])){
			throw_exception('请配置您申请的APP_KEY和APP_SECRET');
		} else {
    ...
    public static function getInstance($type, $token = null) {
        ...
        import("Org.ThinkSDK.".$name);
    	if (class_exists('ORG\\ThinkSDK\\'.$name)) {
    		$class = 'ORG\\ThinkSDK\\'.$name;
    		return new $class($token);
    	} else {
    		throw_exception(L('_CLASS_NOT_EXIST_') . ':' . $name);
    	}
    }
