...
namespace Addons\QuickLogin\Event;
...
class TypeEvent{
	...
	public function weixin($token){
		...
		if(!isset($data['errcode'])){
            ...
            $userInfo['open_id'] = $data['unionid'];
			$userInfo['name'] = $data['nickname'];
			$userInfo['nick'] = $data['nickname'];
			$userInfo['head'] = $data['headimgurl'];
			return $userInfo;
		} else {
			E("获取微信用户信息失败：{$data['errmsg']}");
		}
	}

   	//登录成功，获取微信用户信息
	public function weixingz($token){
		import("ORG.ThinkSDK.ThinkOauth");
		$weixin   = ThinkOauth::getInstance('weixingz', $token);
		$data = $weixin->call('sns/userinfo');
        
		if(!isset($data['errcode'])){
			$userInfo['type'] = 'WEIXIN_GZ';
            $userInfo['open_id'] = $data['unionid'];
			$userInfo['name'] = $data['nickname'];
			$userInfo['nick'] = $data['nickname'];
			$userInfo['head'] = $data['headimgurl'];
