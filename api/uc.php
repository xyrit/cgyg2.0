...
if(!defined('IN_UC')) {
	...
	$discuz->init();
    */
	require DISCUZ_ROOT.'/uc_client/config_ucenter.php';
    require DISCUZ_ROOT.'/api/function_core.php';
    include DISCUZ_ROOT.'/Application/Common/Common/function.php';
...
class uc_note {
    ...
    */
	...
	function synlogin($get, $post) {
        ...
        $home_config = include DISCUZ_ROOT.'/Application/Wap/Conf/config.php';    

        $cookietime = time()+3600*24*7;
        setcookie($home_config['COOKIE_PREFIX'].'user_auth', json_encode($auth), $cookietime,'/');
        //setcookie($home_config['COOKIE_PREFIX'].'uid', $auth["uid"], $cookietime,'/');
        setcookie($home_config['COOKIE_PREFIX'].'user_auth_sign', data_auth_sign($auth), $cookietime,'/');
	}
	...
	function updateapps($get, $post) {
		...
		if($UC_API && is_writeable(DISCUZ_ROOT.'/uc_client/config_ucenter.php')) {
			if(preg_match('/^https?:\/\//is', $UC_API)) {
				$configfile = trim(file_get_contents(DISCUZ_ROOT.'/uc_client/config_ucenter.php'));
				$configfile = substr($configfile, -2) == '?>' ? substr($configfile, 0, -2) : $configfile;
				$configfile = preg_replace("/define\('UC_API',\s*'.*?'\);/i", "define('UC_API', '".addslashes($UC_API)."');", $configfile);
				if($fp = @fopen(DISCUZ_ROOT.'/uc_client/config_ucenter.php', 'w')) {
