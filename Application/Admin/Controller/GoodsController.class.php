...
class GoodsController extends AdminController {
    ...
    public function index($cate_id = null, $model_id = null, $position = null, $group_id = null) {
        ...
        //分页
        $count = M('shop')->where($where)->count(); //总记录数
...
//        }
    ...
    public function edit() {
        ...
        //获取当前分类的文档类型
        $this->assign('type_list', get_type_bycate($data['cat_id']));
    ...
    public function actions() {
        ...
        if ($data['id']) {
            ...
            $rs1 = $o_info->where('id=' . $data['id'])->save($data);

            $rs2 = M('shop_detail')->where('id=' . $data['id'])->save($data);

            $this->success('更新成功', 'Goods/index');
        } else {
            //新增
            $data['add_time'] = time();
            $data['update_time'] = time();
    ...
    //商品上架
    function grounding($id_arr) {
        $ids = array();
        $ids = I("ids")?I("ids"):$id_arr;    //批量处理
        $id = I("id", 0);
        if ($id > 0) {
            $ids[0] = $id;
        }
    ...
    // 下架
    public function grounding_down() {
        $gid = $_GET['id'];
        $pdata = M('shop')->where('id=' . $gid)->find();
        if (!$pdata) {
            $this->error("不存在该商品,下架失败！");
        }
        if ($pdata['stocks'] != 1) {
            $this->error("商品已经下架！");
        }

        $rs2 = M('shop')->where('id=' . $gid)->save(array('stocks' => 0));
        if ($rs2) {
            $this->success("下架成功！");
        } else {
            $this->error("下架失败！");
