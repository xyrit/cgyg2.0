...
class GoodsController extends HomeController
{
           ...
           //查询图文详情
            ...
            if ($data['uid']) {
                ...
                foreach ($lottery_orders as $key => $row1) {
                    ...
                    $this->assign('lottery_orders',$lottery_orders);    //查询所有记录
    ...
    public function detail_info()
    {
        ...
        $gid=I('gid',643);
        $infos= M('shop_detail')->where('id='.$gid)->find();
    ...
    public function list_data()
    {
        ...
        $params['order']=$sort;
        $params['field']=$field;
        $goods_data = D('goods')->getGoodsList($page,$pagesize,$params);
        if($goods_data['data']){
    ...
    public function purchase_records()
    {
       ...
       $list=  M('lottery_attend la')->field('la.id,la.create_date_time,m.face,m.nickname,m.uid,la.attend_count,la.attend_ip')->join('cg_member m on m.uid=la.uid')->where($arr1)->limit($pageIndex,$count)->select();
    ...
    public function to_announce()
    {
        ...
        $list = M('lottery_product lp')->field('lottery_time,pid,status,lottery_id,lottery_code,uid,expecttime')->where('status >0 and pid=' . $pid)->limit($pageIndex, $count)->order('lp.lottery_pid desc')->select();
    ...
    public function CalculationRules()
    {
        ...
        $purchase_list= M('lottery_attend la')->field('la.uid,la.create_date_time,la.sfm_time,m.nickname,la.attend_count,la.lottery_id,doi.title')->join('cg_member m on la.uid=m.uid')->join('cg_shop doi on doi.id=la.pid')->where('la.create_time <'.$data['last_attend_time'])->limit(0,50)->order('la.pay_time desc')->select();


         $purchase_time_sum =M('lottery_attend la')->where('la.pay_time <'.$data['last_attend_time'])->limit(0,50)->sum('la.sfm_time');
    ...
    public function newest_data_list(){
        ...
        $type=I('get.type','');
        $params = array();
        $goods_data = D('goods')->getNewest($page,$pagesize,$params);
    ...
    public function queryInfo()
    {
        ...
        if($data['status'] ==2) {
            ...
            $result['face']=getUserInfo($data['uid'],'face');
