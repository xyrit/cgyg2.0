...
class GoodsController extends HomeController
{
    ...
    public function detail()
    {
        ...
        if($data['status']==0)            //未开奖
        {
           //查询图文详情
            ...
            {
               $data['nowtime']=time();
                $data['expecttime']=date('Y/m/d h:i:s', $data['expecttime']);
            ...
            if ($data['uid']) {
                    ...
                    $userInfo['lottery_time']=$data['lottery_time'];   //开奖时间
    ...
    public function to_announce()
    {
        ...
        $list = M('lottery_product lp')->field('pid,status,lottery_id,lottery_code,uid,expecttime')->where('status >0 and pid=' . $pid)->limit($pageIndex, $count)->select();
    ...
    public function CalculationRules()
    {
        ...
        $purchase_list= M('lottery_attend la')->field('la.create_date_time,la.sfm_time,m.nickname,la.attend_count,la.lottery_id,doi.title')->join('cg_member m on la.uid=m.uid')->join('cg_shop doi on doi.id=la.pid')->limit(0,50)->order('la.create_time desc')->select();
         $purchase_time_sum =M('lottery_attend la')->limit(0,50)->sum('la.sfm_time');
    ...
    //所有参与记录
    public function PurchaseRecordes()
    {
    ...
    public function query(){
        ...
        foreach($data as $v){
            ...
            if($v['status'] ==2){
                ...
                $userinfo=unserialize($v['userinfo']);
                $result[$v['lottery_id']]['attend_count']=$userinfo['attend_count']?$userinfo['attend_count']:0;
