...
function getpage($count, $pagesize = 10) {
    ...
    // $aa = '<li class="rows">共<b>%TOTAL_ROW%</b>条记录&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li>';
    $aa = '<li class="count-box">共<b>%TOTAL_ROW%</b>条记录&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li>';
    $p->setConfig('header', $aa);
    $p->setConfig('prev', '<li><a href=""><span class="icon-angle-left"></span>上一页</a></li>');
    $p->setConfig('next', '<li><a href=""><span class="icon-angle-right"></span>下一页</a></li>');
    $p->setConfig('last', '末页');
...
function transForm($soso = 0, $table = 0) {
    switch ($soso) {
        case 0:
            break;
        case 1:
    ...
    return($soso > 0) ? " and (" . $table . " BETWEEN " . $timeStart . " AND " . $timeEnd . ")" : "";
}
...
function is_login() {
    $user = json_decode(cookie('user_auth'), true);
    //$user["user_id"] = 10000065;
    if (empty($user)) {
        return 0;
    } else {
       // return 10000065;
        return cookie('user_auth_sign') == data_auth_sign($user) ? $user["user_id"] : 0;
    }
}
...
function getUserInfo($id, $field = null) {
    ...
    /* 读取缓存数据 */
    if (empty($list)) {
        $list = S('sys_userinfo_list');
    }


    /* 获取用户uid */
    if (!isset($list[$id])) {
        $userinfo = M('Member')->field('nickname,uid,face,mobile,status')->find($id);

        if (!$userinfo ) { //不存在用户，或用户被禁用
            return '';
        }
        $list[$id] = $userinfo;
        S('sys_userinfo_list', $list); //更新缓存
    }
 ...
 * @return string
 */
function randomString($type = 0, $length) {
    $str_config = array(array('count' => 10, 'str' => '0123456789'), array('count' => 36, '0123456789abcdefghijklmnopqrstuvwxyz'));
    $str_list = "";
    for ($i = 0; $i < $length; $i++) {
        $rand = rand(0, $str_config[$type]['count'] - 1);
        $str_list .= $str_config[$type]['str'][$rand];
    }
    return $str_list;
}
...
function get_addr() {
    $getIp = $_SERVER["REMOTE_ADDR"] ? $_SERVER["REMOTE_ADDR"] : '202.96.134.133';
...
function build_order_no() {
    ...
    return date('ymdhms', time()) . substr(microtime(), 2, 3) . mt_rand(1000, 9999);
}
...
function build_pay_no() {
    return 'Z' . date('ymdhms', time()) . substr(microtime(), 2, 3) . mt_rand(1000, 9999);
}

/**
 * 获取绝对地址
 */
function getfullImg($url='',$type=''){
    if(!$url){
        return '';
    }
    $endfile='';
